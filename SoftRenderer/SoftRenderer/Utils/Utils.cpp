...
{
	_Vector3D eye(eyeX, eyeY, eyeZ);

	_Vector3D target(targetX, targetY, targetZ);

	_Vector3D up(upX, upY, upZ);

	_Vector3D viewDir = target - eye;
	viewDir.Normalized();
	_Vector3D n = viewDir;

	_Vector3D u = n.Cross(up);
	u.Normalized();

	_Vector3D v = u.Cross(n);
	v.Normalized();
